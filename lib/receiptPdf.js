...
function renderReceiptPdf(receipt) {
  return new Promise((resolve, reject) => {
    ...
    // item) checkout rather than a bare custom amount - shown after the
    // main transaction details (Merchant/Reference/Amount/etc. above),
    // each on its own row: name left-aligned (wraps if long), qty x unit
    // price and line total right-aligned, followed by a bold Total row.
    if (itemCount) {
      doc.moveDown(0.5);
